...
from util.load_data_avazu import Loader, INFO_LOG
...
def main(_):
    ...
    loader = Loader(flag="avazu")
    config = Config(loader, flag="avazu")
    config.gpu = 2
    ...
    with tf.Session(graph=graph, config=session_config) as session:
        ...
        best_epoch_auc = 0.
        maxlen = 20
        dataset_cnt = 0 # how many times the full dataset are used
        ...
        for epoch in range(config.epoch_num):
            ...
            change_freq = 10 - (1 * int(dataset_cnt/20))
            if change_freq < 5:
                change_freq = 5
